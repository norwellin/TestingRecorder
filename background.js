...
function createLoginCode(settings) {
  if (!settings?.enabled) return [];

  const useEnvironment = settings.credentialSource === "environment";
  const usernameValue = useEnvironment
    ? "process.env.TEST_USERNAME"
    : JSON.stringify(String(settings.username || ""));
  const passwordValue = useEnvironment
    ? "process.env.TEST_PASSWORD"
    : JSON.stringify(String(settings.password || ""));

  return [
    "// Recorder login setup",
    `await page.locator(${JSON.stringify(settings.usernameLocator)}).fill(${usernameValue});`,
    `await page.locator(${JSON.stringify(settings.passwordLocator)}).fill(${passwordValue});`,
    `await page.locator(${JSON.stringify(settings.loginButtonLocator)}).click();`
  ];
}

function removeGeneratedLoginCode(codeBody) {
  const lines = Array.isArray(codeBody) ? [...codeBody] : [];
  const markerIndex = lines.findIndex(line =>
    String(line).trim() === "// Recorder login setup"
  );
  if (markerIndex >= 0) {
    const blockLength = String(lines[markerIndex + 4] || "").trim() ===
      "await page.waitForLoadState('networkidle');"
      ? 5
      : 4;
    lines.splice(markerIndex, blockLength);
    return lines;
  }

  // Remove login code generated by older versions that had no marker.
  const legacyUsernameLineIndex = lines.findIndex(line =>
    String(line).includes(".fill(process.env.TEST_USERNAME)")
  );
  if (legacyUsernameLineIndex >= 0) lines.splice(legacyUsernameLineIndex, 4);
  return lines;
}

function insertLoginCodeAfterNavigation(codeBody, settings) {
  const lines = removeGeneratedLoginCode(codeBody);
  const loginCode = createLoginCode(settings);
  if (!loginCode.length) return lines;

  const navigationIndex = lines.findIndex(line => /\.goto\(/.test(String(line)));
  if (navigationIndex < 0) return lines;
  lines.splice(navigationIndex + 1, 0, ...loginCode);
  return lines;
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  const debugMessage = message?.loginSettings?.password
    ? {
        ...message,
        loginSettings: { ...message.loginSettings, password: "[REDACTED]" }
      }
    : message;
  console.log("background received:", debugMessage);
  ...
  (async () => {
    if (message.type === "START_RECORDING") {
      ...
      let codeBody = Array.isArray(data.generatedCodeBody)
      ...
      const loginCodeSettings = {
        enabled: message.loginSettings?.enabled === true,
        usernameLocator: message.loginSettings?.usernameLocator || "",
        passwordLocator: message.loginSettings?.passwordLocator || "",
        loginButtonLocator: message.loginSettings?.loginButtonLocator || "",
        credentialSource: message.loginSettings?.credentialSource === "environment"
          ? "environment"
          : "direct",
        username: message.loginSettings?.username || "",
        password: message.loginSettings?.password || ""
      };
      codeBody = insertLoginCodeAfterNavigation(codeBody, loginCodeSettings);
    ...
    if (message.type === "STOP_RECORDING") {
      ...
      const currentActions = Array.isArray(data.generatedAction)
        ? data.generatedAction
        : [];
      const pauseCode = "await page.pause();";

      // Prevent a rapid double-click from appending consecutive pause calls.
      if (codeBody[codeBody.length - 1] !== pauseCode) {
        codeBody.push(pauseCode);
      }
    ...
    if (message.type === "APPEND_RECORD_DATA") {
      ...
      // Navigation is generated asynchronously after Start. Insert the login
      // setup as soon as the first goto line is available.
      codeBody = insertLoginCodeAfterNavigation(codeBody, data.loginCodeSettings);
